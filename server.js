// server.js — Storm Hub completo com Discord Bot opcional (pronto para Render)
require('dotenv').config();

// ============================================================
// VALIDAÇÃO DE AMBIENTE (apenas essenciais para o site)
// ============================================================
...
// ============================================================
// CONFIGURAÇÕES (todas do ambiente)
// ============================================================
...
async function initDatabase() {
  ...
  try {
    ...
    // Migrações para adicionar colunas caso já existam
    await client.query(`
...
async function logKeyAction(action, key, message) {
  try {
    ...
    console.error('Erro ao logar ação de key:', err.message);
  }
}
...
app.get('/api/scripts', auth, async (req, res) => {
  ...
  if (tag) {
    conds.push(`s.id IN (SELECT script_id FROM script_tags WHERE tag_id = $${params.length + 1})`);
    params.push(tag);
  }
